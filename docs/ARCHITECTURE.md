...
**무엇을.** 단일 Spring Boot 프로세스 안에 14개 바운디드 컨텍스트(User, Project, Analysis, Graph, …)를 두고, 컨텍스트 간 경계를 코드 규칙(§10)으로 강제한다. 각 컨텍스트는 독립 배포가 가능한 수준으로 설계한다.
...
### 11. PR 게이트 등급제 — correctness(0단계)/architecture(1단계)/experimental(2단계)

**무엇을.** HIGH 경고를 "작동하는가"(correctness)와 "잘 짜여졌는가"(architecture/experimental) 두 축으로 나눠 게이팅 여부를 다르게 정한다. 0단계(실행 시점에 실제로 깨지는 버그 — MISSING_TRANSACTIONAL_DELETE·ASYNC_SELF_CALL)는 프로젝트 설정과 무관하게 항상 PR을 막는다. 1단계(검증된 구조 위반 8종)는 기본 켜짐이되 프로젝트가 끌 수 있다. 2단계(교차 프로젝트 검증이 아직 부족한 신규 룰 — INTERFACES_IMPORTS_INFRA)는 기본 꺼짐, 옵트인.

**쉽게 말하면.** "코드가 스파게티여도 작동만 하면 절대 안 막는다"는 원칙과 "그래도 검증된 구조 원칙은 지켜야 한다"는 원칙이 충돌하는 지점을, 룰을 성격별로 3단으로 나눠서 풀었다. 실행 안 되는 버그는 예외 없이 막고, 설계 취향 문제는 팀이 선택하게 하고, 아직 신뢰가 안 쌓인 신규 룰은 원하는 팀만 미리 적용받는다.

**어떻게 도달했나(원문 보존, 당시 배경).** 이 표에 도달하기 전 4개 대안(단순 이분법 승격 → Loose/Strict 프로젝트 토글(룰 목록 하드코딩) → (프로젝트,룰)별 위반 건수 래칫 → 줄 단위 diff-scope)을 순서대로 검토·기각했다 — 각각 "분류 기준이 자기 프로젝트 이력에 편향됨", "보편적 게이트가 아니라 팀별 맞춤 게이트가 됨", "IMPORT 기반 룰 대다수가 줄 번호를 안 갖고 있어 정밀 적용 불가"라는 구체적 이유로 걸렸다. "작동 여부"와 "설계 품질"이 다른 축이라는 인식이 마지막 돌파구였다.

**깨지면 생기는 일.** 이 구분이 없으면 두 가지 실패 모드 중 하나로 수렴한다 — 모든 HIGH를 무조건 게이팅해 레거시 프로젝트가 도입 즉시 마이그레이션을 강제당하거나(이탈), 반대로 전부 느슨하게 둬 실제 크래시 버그까지 안 막히거나(신뢰 상실).

**원문 기록.** decisions/DECISIONS_BACKEND.md "PR 게이트 0/1/2단계 등급제 도입"(2026-07-14).

### 12. i18n — react-i18next + 네임스페이스별 JSON, 페이지 단위 점진 이관

**무엇을.** 사용자에게 보이는 화면 텍스트만 `react-i18next`로 다국어화(코드 주석·로그·decisions 문서 등 개발자용 한국어는 그대로 유지). 브라우저 언어 자동감지(`i18next-browser-languagedetector`) + `AppHeader`의 수동 토글 + `localStorage` 저장 3단 구성. 번역 키는 페이지/영역 단위 네임스페이스(`common`·`landing`·`misc`·`legal`)로 분리해 파일당 관리 단위를 작게 유지하고, 전체 페이지를 한 번에 옮기지 않고 공개 페이지→내부 페이지 순으로 점진 이관한다.

**쉽게 말하면.** 통역사(i18next)를 한 명 고용해 화면에 걸린 안내판만 번역시킨다. 안내판마다 담당 구역(네임스페이스)을 나눠서, 한 안내판 번역이 다른 안내판 파일을 건드리지 않게 한다. 방문객 모국어를 자동으로 알아채되, 원하면 직접 언어를 바꿀 수 있는 스위치도 입구에 둔다.

**어떻게 도달했나.** 처음 요청은 "포트폴리오 노출용 랜딩페이지 카피만" 수준의 가벼운 국제화였으나, 논의 중 "해외 채널에 공유했을 때 사용법 자체를 이해 못 하면 의미 없다"는 지적으로 로그인 후 화면까지 포함한 전체 스코프로 확장됐다(2026-07-15, codeprint_129). 페이지 51개·한글 포함 2,600여 줄 규모를 실측 확인 후, 한 번에 다 옮기는 대신 페이지 우선순위(공개 페이지·법적 문서 → 핵심 기능 화면 → 서사형 장문 페이지 순)로 나눠 PR을 여러 개로 쪼개기로 함 — 대규모 리팩토링을 한 PR에 담으면 회귀 위험과 리뷰 난이도가 함께 커진다는 판단. 법적 문서(약관·개인정보처리방침)는 번역 오류가 실제 법적 리스크가 될 수 있어 영어판에 "충돌 시 한국어 원문 우선" 고지를 추가하는 절충안을 택했다(전문 번역 검수는 별도 과제로 보류).
...
### 14. 프론트 테스트 인프라 — Vitest + React Testing Library, 컴포넌트 렌더링 없이 순수 로직부터

**무엇을.** `vite.config.ts`를 `/// <reference types="vitest/config" />`로 확장해 별도 설정 파일 없이 Vitest를 얹었다. React Testing Library·jest-dom도 함께 설치해뒀지만, 첫 테스트는 컴포넌트를 렌더링하지 않는 **순수 함수 단위 테스트**로 잡았다 — `GraphPage.tsx`에 중복돼 있던 "opaque 레이어의 group→file→function 3단계 hidden 계산" 로직을 `graphLayout.ts`의 `applyLayerModeNodeVisibility`로 추출해 테스트했다.

**쉽게 말하면.** 그래프 화면 전체(라우터·API·WebSocket·다국어까지 다 갖춰야 렌더링되는 무거운 컴포넌트)를 통째로 테스트 환경에 띄우는 대신, "레이어를 흐리게 하면 그 안의 그룹·파일·함수가 전부 숨겨져야 한다"는 계산 로직만 화면에서 떼어내 순수 함수로 만들고 그 함수만 테스트했다. 부품 하나를 조립 라인에서 떼어내 따로 검사하는 것과 같다.

**어떻게 도달했나.** 프론트 테스트가 0건인 상태에서 첫 테스트를 "제대로 된 회귀 방지"로 만들려면 실제 재발 이력이 있는 버그를 골라야 한다는 게 출발점 — `ERROR_TRACKER.md` [반복-C](React Flow `extent:'parent'` hidden 미전파)가 유일한 후보였다. 코드를 열어보니 이 버그가 두 번 재발한 이유가 드러났다: `applyPresetConfig`(프리셋 로드)와 `toggleLayerOpaque`(범례 클릭)에 **거의 같은 3단계 hidden 계산 로직이 따로 복붙**돼 있었다 — 한쪽을 고쳐도 다른 쪽엔 안 미치니 재발할 수밖에 없는 구조였다. 그래서 테스트를 추가하기 전에 먼저 이 로직을 하나의 순수 함수로 통합했다 — 이러면 테스트 하나가 두 호출부 전부를 보호하고, 셋업도(Provider 없이 입력→출력만 검증) 훨씬 가볍다. 가드를 일시 제거해 테스트가 실제로 FAILED로 떨어지는 것까지 확인해 이 테스트가 진짜 그 버그를 잡는지 검증했다.

**깨지면 생기는 일.** 이 함수 없이 로직이 다시 두 곳에 흩어지면, 한쪽만 고치고 다른 쪽을 잊는 실수가 세 번째로 재발할 수 있다 — 함수 통합 자체가 재발 표면을 하나로 줄이는 방어책이고, 테스트는 그 하나 남은 지점이 깨지지 않았는지를 계속 확인한다.

**원문 기록.** decisions/DECISIONS_FRONTEND.md "프론트 테스트 인프라 도입 — Vitest + React Testing Library"(2026-07-25).
