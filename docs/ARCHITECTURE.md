...
### 10. AI 컨텍스트 생성기 — 웹/MCP 단일 소스화 (RepoMapService)

**무엇을.** 그래프를 파일/함수 트리 마크다운으로 바꾸는 로직을 `application/graph/RepoMapService`(백엔드) 한 곳에만 두고, 웹 다운로드 버튼과 MCP `get_repo_map` 툴이 같은 서비스를 호출한다.

**쉽게 말하면.** 예전엔 이 "지도 그리기" 로직이 프론트(TS) 안에만 있어서, AI 에이전트용 출구(MCP)를 새로 내려면 같은 그림을 Java로 또 그려야 했다. 지도를 그리는 사람을 하나로 통일하면, 지도가 바뀔 때 한 번만 고치면 모든 출구에 반영된다.

**어떻게 도달했나.** PRODUCT_STRATEGY.md §16이 MCP 토큰 절감 채널의 본체로 `get_repo_map`을 지목했는데, 기존 생성 로직이 프론트 클라이언트 함수(`downloadTreeText`)뿐이라 그대로 두면 백엔드에 같은 알고리즘을 중복 구현하게 되는 상황이었다. 이식 과정에서 프론트 원본의 잠재 버그(공통 조상 디렉터리에 파일이 직접 있으면 트리에서 누락)를 발견했고, 웹/MCP 양쪽에 동일하게 적용될 사양이므로 프론트를 그대로 베끼는 대신 이번에 바로잡았다(재귀 진입점을 하나로 통합하는 것으로 해결 — 코드도 더 단순해짐).

**깨지면 생기는 일.** 생성 로직이 프론트/백엔드 두 곳에 있으면 포맷 개선이 한쪽에만 반영되고, 다음 배포 형태(§16.2 ③레포 커밋 파일·④데스크탑 자동 갱신)를 추가할 때마다 세 번째·네 번째 구현이 늘어난다.
