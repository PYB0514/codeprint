...
- **방법**: 머지 전 `/code-review`(보안 리뷰 모드) 또는 신선한 컨텍스트의 Agent 호출로 다음을 전담 검토 — ① 시크릿/자격증명 노출 여부(바이너리 산출물이면 실제로 풀어서 확인) ② 과도한 권한 부여 ③ 새로운 공개 노출 채널
- 통과해야만 머지 진행. 통과 못 하면 원인 수정 후 재검증.
