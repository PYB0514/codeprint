...
**세션 종료 프로세스 — 트리거는 사용자의 마무리 신호(§9)뿐이다.**
(과거에는 PostCompact 훅 신호도 트리거였으나 2026-07-07 사용자 결정으로 훅 자체를 제거 — 이제 압축 전에 세션이 끝나는 경우가 대부분이라 실효성이 없었다.)
...
2. `contexts/Context{N}.md` — 가장 번호가 높은 최신 Context 파일(`ls contexts/ | sort -V | tail`로 직접 찾는다). **"다음 세션 첫 액션"의 단일 소스는 이 파일이다** — PROGRESS.md는 더 이상 이 정보를 담지 않는다(2026-07-10 구조 개편, 근거는 `decisions/DECISIONS_INFRA.md` 참조).
3. `PROGRESS.md` — 기능 백로그·제품 전략 방향·로컬 실행법·알려진 문제(참조용, "다음 액션"은 없음)

이어서 `ToolSearch`로 `codeprint` MCP 연결 여부를 확인한다(연결은 세션 시작 시점 1회뿐이라 지금 확인해두면 이후 반복 확인 불필요) — 안 잡히면 이번 세션은 규칙 4 "자가 진단" 절차(analyzeLocal 대체 + 백엔드를 세션 끝까지 켜둬서 다음 세션 연결 준비)를 따른다.

**"다음은 뭐 할까?" 라고 묻지 않는다.**
작업이 끝나면 최신 Context 파일과 SECURITY_POLICY.md를 보고 스스로 다음 우선순위를 판단해서 진행한다.

**우선순위 판단 기준 (순서대로)**
1. 열려 있는 PR이 있으면 → CI 상태 확인, 머지 가능하면 머지 제안
2. 미해결 보안 항목 → 보안이 기능보다 우선. SECURITY_POLICY.md 체크리스트 + 최신 Context 파일·`ERROR_TRACKER.md`의 보안 백로그 둘 다 확인한다 (공개 레포이므로 미수정 결함의 상세는 로컬 전용 문서에서만 관리)
3. 최신 Context 파일의 "다음 컨텍스트에서 할 것"에 명시된 항목
...
*OAuth 로그인이 필요한 플로우 (2026-07-07 프로세스 변경 — 이전 "사용자에게 직접 테스트 요청"을 대체)*
- GitHub 로그인이 필요한 검증은 **Claude가 claude-in-chrome으로 창을 열고 사용자에게 로그인만 요청**한다. 로그인 후 화면 검증·조작은 Claude가 직접 수행한 뒤 push.

*자가 진단 (codeprint MCP, push 전 항상)*
- push 전 `codeprint` MCP(`get_warnings`)로 이번 변경사항을 자가검사(HIGH/MEDIUM 경고 확인)
- **세션 시작 전 백엔드+DB를 미리 켜둔다.** codeprint MCP(`.mcp.json`, `http://localhost:8080/mcp/rpc`)는 Claude Code가 세션(터미널) 시작 시점에 딱 한 번만 연결을 시도한다 — 세션 도중 `preview_start`로 백엔드를 나중에 켜도 그 세션 안에서는 재연결되지 않는다(하네스 레벨 동작, 에이전트가 재시도를 강제할 수단 없음). 따라서 **직전 세션을 끝낼 때 Docker DB+백엔드를 켜진 채로 남겨두는 것**이 다음 세션에서 MCP가 연결되게 하는 유일한 방법이다.
- **근본 원인이었던 CORS 버그는 수정 완료(2026-07-09, PR 참조)** — `SecurityConfig`의 CORS 화이트리스트(`localhost:3000`·배포 도메인만 허용)가 `/mcp/**`에도 그대로 적용돼, Origin 헤더를 보내는 MCP 클라이언트의 요청 자체가 매번 403으로 거부되고 있었다(`curl -H "Origin: ..."`로 재현·확정). `/mcp/**`는 브라우저 쿠키 인증이 아닌 AI 에이전트 전용 stateless 엔드포인트라 별도 CORS 설정(모든 Origin 허용, `allowCredentials=false`)으로 분리해 해결 — 다른 경로의 화이트리스트는 그대로 유지(회귀 없음, curl로 확인).
  - **세션 시작 직후 확인**: `ToolSearch`로 `codeprint` 관련 툴이 잡히는지 먼저 확인한다.
  - **잡히면**(직전 세션이 백엔드를 켜둔 채 끝났던 경우): 그대로 `get_warnings` 사용.
  - **안 잡히면**(이번이 처음이거나 백엔드가 꺼진 채 세션이 시작된 경우): 이번 세션은 MCP 자가검사를 포기하고 `./gradlew analyzeLocal`(Spring/DB 불필요, 같은 분석 엔진을 직접 실행)로 대체 검증한다. 동시에 Docker DB+백엔드를 `preview_start`로 켜서 **세션이 끝날 때까지 계속 띄워둔다**(불필요하게 내리지 않는다) — 다음 세션부터 자동 연결되도록. 세션 마무리 시 이 사실을 사용자에게 짧게 안내한다.
- **개발 중간 자가검사**: push 직전 1회로 미루지 않는다. 관련 파일을 여러 개 고친 뒤 자연스러운 멈춤 지점(한 기능 단위 완료, 다음 파일로 넘어가기 전 등)마다 `get_warnings`(또는 위 대체 수단)로 중간 점검한다 — 문제를 push 직전이 아니라 만드는 시점에 바로 발견하기 위함
...
- 기능을 추가했다가 제거하거나 번복했다 — **기존 결정을 대체하는 경우 §7 supersede 프로세스(배너+현행 문서 동기화)까지 적용했는지 확인**
...
**결정 대체(supersede) 프로세스 (2026-07-07 도입)**
어떤 결정이든(설계·요금제·정책·기능·프로세스) 기존 결정을 대체하는 새 결정이 나오면, 같은 PR/세션에서 아래 3단계를 수행한다.

1. **새 결정 기록** — decisions/ 해당 파일에 새 항목을 쓰되, 본문 첫머리에 `대체: "<기존 항목 제목>"(원 날짜)`와 번복 이유를 명시한다.
2. **기존 결정에 대체 배너** — 기존 항목 제목 바로 아래에 배너를 단다. 원문은 수정·삭제하지 않는다(이력 = 면접 자료).
   ```
   > ⚠️ **대체됨 (YYYY-MM-DD)** — "<새 항목 제목>"으로 대체. 이유 한 줄. 아래 원문은 이력 보존용.
   ```
3. **현행 문서 동기화** — 그 결정을 서술하는 현행 문서를 전부 새 결정 기준으로 갱신한다. 기존 결정의 키워드(기능명·가격·정책 문구)로 Grep해 스테일 서술을 찾는다.
   - 추적 문서: README, docs/(ARCHITECTURE "구조 채택 이유" 섹션 포함), CLAUDE.md — 교체(과거 원문은 git 히스토리 + decisions/ 배너가 보존).
   - 로컬 문서: PROGRESS.md `📋 기능 백로그`에 스테일 서술이 있으면 제거(§9 운용 원칙 재사용).
   - 메모리: 해당 메모리 파일을 갱신하거나 삭제.

**원칙 — 문서는 두 부류이고, 같은 결정이 두 얼굴로 존재하는 게 정상이다.**
- **일지형**(decisions/, contexts/) = append-only 이력. 대체돼도 배너만 달고 보존한다.
- **현행형**(README, docs/, CLAUDE.md, PROGRESS 활성부, 메모리) = 항상 현재 진실 하나만 서술한다. 대체 즉시 교체한다.
- 이 프로세스가 없으면 스테일 서술이 누적된다 — 2026-07-06 문서 위생 패스(PR #456)에서 17건을 한꺼번에 정정해야 했던 것이 그 비용이다. 대체 시점에 1건씩 동기화하는 게 항상 더 싸다.

**소급 적용은 하지 않는다(lazy migration).** 과거 대체된 항목 전수 스윕 대신, 배너 없는 대체된 항목을 발견하면 그 자리에서 배너를 단다.
...
- **gitignore/exclude된 파일(contexts/, PROGRESS.md, PROGRESS_ARCHIVE.md, ERROR_TRACKER.md, INTERVIEW_POINTS.md, PRODUCT_STRATEGY.md, V1_UX_GAP_REVIEW.md, .claude/)은 로컬에서만 수정한다. git add/commit/push 하지 않는다. PR도 만들지 않는다. 공개 레포이므로 전략·미수정 보안 결함의 상세는 반드시 이 로컬 문서들에만 쓴다.**

**커밋 규칙**
- 기능 하나 완성할 때마다 즉시 커밋. 세션 끝에 몰아서 커밋하지 않는다.
- 완료 기록은 커밋마다 PROGRESS.md에 남기지 않는다(2026-07-10부터 — §9 참조) — 세션이 끝날 때 Context 파일에 한 번에 정리한다. 단, 백로그 항목을 완료했다면 PROGRESS.md `📋 기능 백로그`에서 그 항목은 즉시 삭제한다.
...
[ ] 완료한 항목이 이번에 작성할 Context 파일의 "완료한 작업"에 반영될 예정인가? (PROGRESS.md엔 안 쓴다 — 아래 참조)
[ ] PROGRESS.md `📋 기능 백로그`에 이번 세션에 완료된 항목이 남아있지 않은가? → 있으면 즉시 삭제(git 히스토리에 남으므로 ✅ 표시로 보존 안 함)
...
**PROGRESS.md 운용 원칙 (2026-07-10 구조 개편)**
> 예전엔 "🚀 다음 세션 첫 번째 액션" 섹션에 매 세션 서사를 append하다 600줄 넘으면 아카이빙하는 방식이었으나, 그 섹션이 Context 파일과 거의 100% 중복이라 폐지했다(근거: `decisions/DECISIONS_INFRA.md`). 지금은 훨씬 단순하다.
- **PROGRESS.md는 세션 로그를 담지 않는다.** "다음 액션"은 오직 Context 파일에만 쓴다(바로 아래 항목).
- **📋 기능 백로그는 완료 즉시 삭제한다.** ✅ 표시로 남겨두지 않는다 — git 히스토리·decisions/가 이미 그 기록을 갖고 있다. 임계값(줄 수) 판단 없이 매번 그냥 정리한다.
- 그래도 대량 서술을 보존해야 할 특수한 경우만 `PROGRESS_ARCHIVE.md`(로컬 전용, append-only, `## [아카이빙 YYYY-MM-DD] <섹션명>` 헤더로 원문 그대로 추가)를 쓴다 — 일반적인 완료 항목 정리는 아카이빙 없이 그냥 삭제.

**Context 파일 생성 후 반드시 할 것**
1. **새 Context 파일 자체가 "다음 세션 첫 액션"의 유일한 소스다** — 별도로 PROGRESS.md를 갱신할 필요 없음(위 원칙 참조). PROGRESS.md는 📋 백로그에 이번 세션에 완료된 항목이 남아있으면 그것만 정리한다.
