...
- **빌드 산출물(jar/zip/이미지/패키지 등)을 만드는 태스크를 새로 추가·수정할 때는, 로컬 전용 secret 파일(`application-local.yml` 등)을 기본적으로 제외했는지 확인 — "문제되면 나중에 뺀다"가 아니라 처음부터 opt-in(명시적으로 필요한 것만 포함)으로 짠다(2026-07-12 fat jar 자격증명 유출 사고, `decisions/DECISIONS_INFRA.md` 참조)

**공개 배포 채널 신규 개설 — 하드 게이트(예외 없음)**
새 공개 저장소·패키지 레지스트리 등 "코드가 아닌 산출물이 외부로 나가는 채널"을 처음 만들 때는, 첫 게시 전 반드시:
1. 산출물(바이너리 포함) 내부 파일 목록을 직접 열어서 확인
2. 그 채널에 바이너리 내부까지 실제로 풀어서 검사하는 자동 스캔 CI 게이트를 먼저 구축
게이트 없이 "일단 게시하고 나중에 게이트 추가"는 금지 — 순서를 반드시 게이트 먼저, 게시는 그 다음.
