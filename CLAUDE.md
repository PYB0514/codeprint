...
- 빌드 산출물(jar/zip/이미지/패키지 등)을 만드는 태스크는 리소스를 **opt-in**으로 짠다 — "전체 포함 후 위험한 것만 제외"가 아니라 "필요한 것만 명시적으로 포함"이 기본값(12-factor 원칙 — config는 코드/산출물과 분리, 산출물만 봐서는 어떤 시크릿도 알 수 없어야 함)
- 공개 저장소·패키지 레지스트리 등 외부로 나가는 산출물은 **최초 게시 전 반드시 내부 파일 목록을 직접 열어서 확인**한다. 반복 실행되는 전용 CI 스캐너를 매번 새로 만드는 건 이 프로젝트 규모에선 과설계(오버헤드만 크고 놓치는 경로는 여전히 남음) — 대신 아래 시크릿 관리 원칙(환경별 자격증명 분리)으로 "새도 잃을 게 없는" 상태를 기본값으로 만든다. GitHub Secret Scanning/Push Protection(플랫폼 자체 기능, 실행 비용 0)은 상시 켜둔다.
- **환경별 자격증명 분리(OWASP Secrets Management Cheat Sheet 원칙)** — 로컬 개발은 프로덕션 실 자격증명을 절대 재사용하지 않는다. 결제·OAuth 등 제공자가 테스트/샌드박스 키를 지원하면 로컬은 반드시 그것만 사용, AWS 등은 로컬 전용 최소권한 자격증명을 별도 발급한다.
- 시크릿 유출이 실제로 발생하면 `SECURITY_POLICY.md`의 "시크릿 유출 대응 절차" 순서를 따른다(탐지→범위 확정→격리→**즉시 재발급**→복구→모니터링→사후 기록 — 히스토리 정리는 위생 조치일 뿐 재발급보다 우선순위가 낮다).
...
- [ ] (해당 시) 보안/인프라/공개배포 카테고리 PR — 독립 적대적 검증 에이전트 통과: [아래 기준·결과]
```
> 브랜치 보호 확인 항목 근거: main 보호 규칙 부재로 CI 체크가 전부 무력화됐던 전례(GATE_GAPS.md [G-2]) 재발 방지.

**보안/인프라/공개배포 — 독립 적대적 검증 필수**
> 근거: 솔로 개발 체제라 GitHub의 "최소 1명 승인" 브랜치 보호는 현실적으로 불가능(승인할 제2의 사람이 없음). 대신 "같은 세션·같은 맥락이 자기 작업을 스스로 재검토"가 아닌, **완전히 새로운 컨텍스트의 에이전트를 별도로 호출**해 사람 리뷰어의 역할(자기 눈엔 안 보이는 걸 다른 눈으로 보기)을 대체한다(2026-07-12 fat jar 자격증명 유출 사고 후 도입, `decisions/DECISIONS_INFRA.md` 참조).

**⚠️ 발동 시점은 "머지 전"에 한정하지 않는다.** 실제 사고는 `codeprint` 메인 레포의 PR→머지 흐름이 아니라 **완전히 새로운 공개 레포(`codeprint-plugins`)를 만들어 그 최초 커밋을 바로 push**하는 경로로 일어났다 — 신규 레포의 첫 커밋은 브랜치 보호도 없고 PR/머지 자체를 안 거치는 게 흔하다. "머지 전"으로만 못박으면 이 경로에선 규칙이 걸릴 지점 자체가 없다. 그래서 발동 지점을 아래로 명시한다:
- 기존 레포의 PR을 main에 머지하기 전
- **신규 저장소·패키지 레지스트리 등을 만들어 그 안에 무엇이든 최초로 공개(첫 push, 첫 게시)하기 전** — PR 과정을 거치는지 여부와 무관하게 적용

- **대상(명시 목록)**: `SecurityConfig` 변경, 새 클라우드 자격증명/IAM 관련 코드, 공개 저장소·패키지에 게시되는 산출물을 만드는 빌드 설정, 신규 `permitAll()`, 새 공개 배포 채널 개설
- **대상(일반 조건, 목록보다 우선)**: **이 프로젝트가 지금까지 한 번도 해본 적 없는 종류의 배포·인프라·외부 노출 행위는 목록에 없어도 무조건 트리거한다.** 오늘 사고 자체가 "체크리스트에 없던 새로운 종류의 행위"였다는 게 근거 — 명시 목록은 이미 아는 위험만 잡고, 다음에 생길 새로운 위험은 "처음 해보는 종류인가?"라는 질문 자체로 잡는다. 애매하면 트리거하는 쪽으로 판단한다.
- **방법**: 위 발동 시점 이전에 `/code-review`(보안 리뷰 모드) 또는 신선한 컨텍스트의 Agent 호출로 다음을 전담 검토 — ① 시크릿/자격증명 노출 여부(바이너리 산출물이면 실제로 풀어서 확인) ② 과도한 권한 부여 ③ 새로운 공개 노출 채널
- 통과해야만 진행(머지든 최초 push든). 통과 못 하면 원인 수정 후 재검증.
...
**문서화(규칙·정책 등) 작업은 논의 중엔 PR·머지 안 함** — 사용자와 대화로 규칙/정책을 다듬는 중(아직 확정 안 됨)에는 매 수정마다 PR을 열고 머지하지 않는다. 같은 브랜치에 커밋만 계속 쌓다가, **사용자가 "이제 확정" 또는 논의 종료를 명시한 시점에 한 번만 PR·머지**한다(2026-07-12, 보안 규칙 재정비 중 PR을 매번 날려 불필요하게 쌓인 사례 이후 도입).
