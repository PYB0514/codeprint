...
- **마이너 업데이트** (소기능 추가, UI 수정, 버그 수정): 현재 버전의 소수점 세 자리 증가
  - 예: `v1.5` 계열이면 → `v1.501`, `v1.502`, `v1.503` ...
- **주요 기능 변경** (새 도메인 기능, 결제/인증/인프라 변경 등): 소수점 첫째 자리 증가
  - 예: `v1.5` → `v1.6` → `v1.7` ...
- 판단 기준 — 아래 중 하나라도 해당하면 주요 버전:
  - 새 Bounded Context(도메인) 추가
  - 결제·인증·배포 인프라 변경
  - 사용자가 체감하는 핵심 흐름(로그인, 분석, 그래프, 결제) 변경
  - DB 스키마 변경(Flyway 마이그레이션 포함)
- 태그 명령: `git tag -a v1.502 -m "v1.502 — 설명"` → `git push origin v1.502`
