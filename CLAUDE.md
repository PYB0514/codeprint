...
**세션 종료 프로세스 — PostCompact 훅 신호가 유일한 트리거**
PostCompact 훅으로부터 신호를 받으면 **1회라도** 즉시 아래 순서로 실행한다.
(압축 횟수 카운팅은 정확하지 않으므로 1회 압축 = 바로 세션 마무리)
작업이 일단락됐거나 자연스러운 멈춤 지점이라는 이유만으로는 절대 실행하지 않는다.

1. 진행 중인 작업을 커밋 가능한 상태로 마무리
2. 백엔드/프론트엔드 서버 종료 (`Get-Process node,java | Stop-Process -Force`)
3. §9 기준으로 Context 파일 작성 (로컬 저장만)
4. PROGRESS.md 업데이트 (로컬 저장만)
5. 대화창에 아래 문구 출력 — 사용자가 새 세션 시작 시 복사해서 쓸 수 있도록
   ```
...
- **gitignore된 파일(contexts/, PROGRESS.md, ERROR_TRACKER.md, INTERVIEW_POINTS.md, .claude/)은 로컬에서만 수정한다. git add/commit/push 하지 않는다. PR도 만들지 않는다.**
...
- **태그를 붙이는 PR:**
  - `feat:` 중 완전히 새로운 기능 → MINOR 증가
  - `feat:` 중 기존 기능 개선·재설계 → PATCH 증가
  - 사용자에게 보이는 `fix:` → PATCH 증가
  - 서비스 동작에 영향을 주는 인프라·보안 변경 → PATCH 이상
- **태그를 붙이지 않는 PR:**
  - `docs:`, `chore:`, `test:`, `refactor:` — 사용자 입장에서 서비스가 변하지 않음
...
#### 사이드이펙트는 Domain Event로 분리
- 주 흐름(생성/수정/삭제)과 무관한 작업(알림, 통계, 로그)은 이벤트로 분리.
- `domain/[context]/event/[EventName].java` 발행 → `application/[context]/[Handler].java` 구독.
- 이벤트를 사용하면 "알림 실패 시 생성도 롤백"되는 문제를 방지할 수 있다.
