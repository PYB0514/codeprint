...
- "잘 됩니다"를 추측으로 선언하지 않는다. 눈으로 확인한 뒤 선언한다.
...
**⚠️ 절대 금지 — "단순해 보이면 건너뛰어도 된다"는 판단**
- "필드 하나 추가", "조건 하나 추가", "링크 하나 추가" 같은 작은 변경도 2단계를 면제받지 않는다.
- 실제로 작동하는지 확인하기 전까지는 "잘 됩니다"라고 선언하지 않는다.
- 정적 검증(컴파일, 타입체크) 통과 ≠ 기능 정상 동작. 둘은 별개다.
...
- 백엔드 코드를 변경했으면 반드시 서버 재시작 후 테스트 (Spring Boot는 핫리로드 없음)
...
This is the step LLMs skip most often. The smaller the change looks, the more likely this step gets skipped. Don't.
