...
- 빌드 산출물(jar/zip/이미지/패키지 등)은 리소스를 **opt-in**으로 구성한다("전체 포함 후 제외"가 아니라 "필요한 것만 포함")
- 공개 배포 산출물은 **최초 게시 전 내부 파일 목록을 직접 확인**한다. GitHub Secret Scanning/Push Protection은 상시 켜둔다.
- **환경별 자격증명을 분리한다** — 로컬 개발은 프로덕션 실 자격증명을 재사용하지 않는다. 세부 기준 `SECURITY_POLICY.md` "시크릿 관리 원칙".
- 시크릿 유출 발생 시 `SECURITY_POLICY.md` "시크릿 유출 대응 절차"를 따른다(재발급이 히스토리 정리보다 항상 우선).
> 위 4개 항목 도입 배경·근거는 `decisions/DECISIONS_INFRA.md` "CLAUDE.md 보안 거버넌스 규칙 재정비" 참조.
...
**보안/인프라/공개배포 — 독립 적대적 검증 필수** (도입 배경·근거: `decisions/DECISIONS_INFRA.md` "CLAUDE.md 보안 거버넌스 규칙 재정비")
- **발동 시점**(PR 머지 여부와 무관, 아래 중 먼저 오는 시점): 기존 PR을 main에 머지하기 전 / **신규 저장소·패키지 레지스트리 등에 무엇이든 최초로 공개(첫 push)하기 전**
- **대상(명시 목록)**: `SecurityConfig` 변경, 새 클라우드 자격증명/IAM 관련 코드, 공개 산출물을 만드는 빌드 설정, 신규 `permitAll()`, 새 공개 배포 채널 개설
- **대상(일반 조건, 목록보다 우선)**: 이 프로젝트에서 처음 해보는 종류의 배포·인프라·외부 노출 행위는 목록에 없어도 트리거한다. 애매하면 트리거.
- **방법**: `/code-review`(보안 리뷰 모드) 또는 신선한 컨텍스트의 Agent 호출로 ① 시크릿/자격증명 노출(바이너리는 실제로 풀어서 확인) ② 과도한 권한 부여 ③ 새로운 공개 노출 채널을 검토. 통과해야만 진행.
...
**문서화(규칙·정책 등) 작업은 논의 중엔 PR·머지 안 함** — 확정 안 된 규칙/정책은 매 수정마다 PR을 열지 않는다. 같은 브랜치에 커밋만 쌓다가 사용자가 확정을 명시한 시점에 한 번만 PR·머지한다(근거: `decisions/DECISIONS_INFRA.md`).
