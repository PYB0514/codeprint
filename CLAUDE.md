...
- **MCP 연결은 세션 시작 시점 1회뿐 — 세션 도중 재연결 불가(2026-07-09 확인).** `.mcp.json`에 codeprint MCP(`http://localhost:8080/mcp/rpc`)가 이미 등록돼 있지만, Claude Code는 세션(터미널) 시작 시점에 딱 한 번만 연결을 시도한다. 그 시점에 백엔드가 꺼져 있었다면 연결은 실패로 끝나고, 세션 도중 `preview_start`로 백엔드를 나중에 켜도 재연결되지 않는다 — MCP 연결은 하네스 레벨 동작이라 에이전트가 재시도를 강제할 수단이 없다.
  - **세션 시작 직후 확인**: `ToolSearch`로 `codeprint` 관련 툴이 잡히는지 먼저 확인한다.
  - **잡히면**(백엔드가 세션 시작 전부터 떠 있던 경우): 그대로 `get_warnings` 사용.
  - **안 잡히면**: 이번 세션은 MCP 자가검사를 포기하고 `./gradlew analyzeLocal`(Spring/DB 불필요, 같은 분석 엔진을 직접 실행)로 대체 검증한다. 동시에 Docker DB+백엔드를 `preview_start`로 켜서 **세션이 끝날 때까지 계속 띄워둔다**(불필요하게 내리지 않는다) — 다음에 사용자가 Claude Code를 재시작하면 그 시점엔 이미 서버가 떠 있으므로 새 세션에서 자동 연결된다. 세션 마무리 시 이 사실을 사용자에게 짧게 안내한다.
