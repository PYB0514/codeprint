...
- **버전 태그를 붙이는 PR** — 사용자가 체감하는 변화가 포함된 PR만:
  - `feat:` — 새 기능, 새 UI
  - `fix:` — 사용자에게 보이는 버그 수정 (화면 깨짐, 기능 오작동 등)
  - 인프라·보안 변경 중 서비스 동작에 영향을 주는 것
- **버전 태그를 붙이지 않는 PR** — 사용자 입장에서 서비스가 변하지 않은 PR:
  - `docs:` — DECISIONS.md, CLAUDE.md, README 등 문서만 변경
  - `chore:` — .gitignore, CI 설정, 의존성 정리 등
  - `test:` — 테스트 코드만 추가
  - `refactor:` — 동작 변경 없는 내부 코드 정리
