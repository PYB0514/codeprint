...
3. **현행 문서 동기화** — 그 결정을 서술하는 현행 문서를 전부 새 결정 기준으로 갱신한다. 기존 결정의 키워드(기능명·가격·정책 문구)로 Grep해 스테일 서술을 찾는다.
   ...
   - 로컬 문서: PROGRESS.md 활성 블록에서 제거하고 분량이 있으면 PROGRESS_ARCHIVE.md로 이동(§9 규칙 재사용).
...
[ ] PROGRESS.md가 600줄을 넘는가? → 넘으면 아래 아카이빙 규칙대로 오래된 블록을 PROGRESS_ARCHIVE.md로 이동
```

모두 통과하면 Context 파일을 작성한다.

**PROGRESS.md 아카이빙 규칙 (로컬 전용 프로세스, 2026-07-07 도입)**
PROGRESS.md가 계속 자라 한 번에 읽을 수 없게 되는 것을 막는 상시 프로세스다. 세션 마무리 자가점검에서 600줄 초과가 확인되면 실행한다.
- **PROGRESS.md에 남기는 것**: 헤더(읽는 법 안내), 🚀 섹션의 최근 세션 블록 3~4개, 활성 백로그·전략 방향·로컬 실행 방법·알려진 문제.
- **PROGRESS_ARCHIVE.md로 옮기는 것**: 그보다 오래된 🚀 세션 블록, 완료 작업 표, 스테일해진 섹션.
- 아카이브는 append-only — 맨 뒤에 `## [아카이빙 YYYY-MM-DD] <섹션명>` 헤더를 달아 원문 그대로 추가하고, 기존 아카이브 내용은 수정·삭제하지 않는다.
- 이동은 잘라내기다(원본에서 삭제 + 아카이브에 추가). 같은 내용을 두 곳에 중복 보존하지 않는다.
- PROGRESS_ARCHIVE.md는 로컬 전용(.gitignore 등록) — 커밋·push 금지(§8 목록 참조).
