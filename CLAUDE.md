...
- 버전 형식: `v{major}.{minor}` — major는 항상 1, minor가 두 가지 레벨로 나뉜다.

| 업데이트 종류 | 규칙 | 예시 |
|---|---|---|
| **주요 기능 변경** | minor 한 자리 증가 | `v1.5` → `v1.6` → `v1.7` |
| **마이너 업데이트** | 현재 주요 버전 + 두 자리 시퀀스 | `v1.6` 이후 → `v1.601` → `v1.602` |

- 마이너 시퀀스는 **항상 현재 주요 버전 기준**으로 시작한다.
  - `v1.6` 이후 마이너: `v1.601`, `v1.602` ...
  - `v1.7` 이후 마이너: `v1.701`, `v1.702` ...
  - `v1.8` 이후 마이너: `v1.801`, `v1.802` ...
- **주요 기능 판단 기준** — 아래 중 하나라도 해당하면 주요 버전:
  - 새 Bounded Context(도메인) 추가
  - 결제·인증·배포 인프라 변경
  - 사용자가 체감하는 핵심 흐름(로그인, 분석, 그래프, 결제) 변경
  - DB 스키마 변경(Flyway 마이그레이션 포함)
- 태그 명령: `git tag -a v1.601 -m "v1.601 — 설명"` → `git push origin v1.601`
...
### 11. DDD Bounded Context Enforcement
