...
**결정.** `GraphBuilderTest`에 동명 함수(`handle`)를 세 개 파일에 중복 정의하고 모호한 전역 폴백 호출을 포함하는 픽스처로 `build()`를 두 번(완전히 독립된 `GraphBuilder`+mock 인스턴스로) 실행해 결과를 비교하는 회귀 테스트 추가. Node는 `(type, name, filePath)`, Edge는 `(edgeIdentifier, type)`을 안정 키로 삼아 비교 — 매 빌드마다 새로 발급되는 랜덤 UUID(`Node.id`/`Edge.sourceNodeId`/`targetNodeId`)는 애초에 비교 대상에서 제외(엔티티 ID가 다른 건 정상이고 결정론 위반이 아니다).

**설계 근거 — 왜 모호한 다중 후보 픽스처인가.** 단순히 파일 2개짜리 입력으로는 이미 결정론적일 가능성이 높다(Java의 `HashMap`은 시드 랜덤화 없이 문자열 해시코드가 고정이라, 같은 삽입 순서면 반복 순서도 보통 재현된다). 진짜 위험 지점은 "여러 후보 중 하나를 고르는" 로직(동명 함수 전역 폴백 후보 선택 등)이 `Set`/`Map` 반복 순서에 암묵적으로 의존하는 경우라, 의도적으로 동명 함수 3개를 만들어 이 경로를 노출시켰다.

**검증.** 신규 테스트 자체가 회귀 가드이자 검증 — 현재 코드베이스에서는 green(현재 결정론 위반 없음을 재확인). 백엔드 전체 테스트 green, `analyzeLocal` 베이스라인 변화 없음.
...
**조사 결과 — 핵심 위반은 이미 잡히고 있다.** `INFRA_LAYER_DIRS`(줄 802~803)가 `"infrastructure", "infra", "persistence", "adapter", "adapters", "dao", "db"`를 전부 별칭으로 묶어둬서, 헥사고날의 정석 위반("domain이 adapter 구현체를 직접 의존")은 **기존 `DOMAIN_IMPORTS_INFRA`가 그대로 감지한다** — adapter가 별도 카테고리가 아니라 infra의 동의어로 이미 취급되기 때문. 게다가 `APPLICATION_LAYER_DIRS`(줄 811)에 `"usecase"`/`"usecases"`가 포함돼 있어, `domain/` + `usecase/` + `adapter/` 3종 디렉터리를 쓰는(흔한 실전 헥사고날 명명) 프로젝트는 `isDddProject()`(2종 이상 발견) 게이트도 별도 코드 변경 없이 이미 통과한다.

**남는 건 신규 감지기가 아니라 순수 라벨링 판단.** 위 결과로 "기존 별칭 재사용만으로 충분한지"(Context133이 제기한 첫 질문)에 대한 답은 **Yes, 충분하다** — 새 `detectXxx` 메서드도, 새 규칙 타입도 필요 없다. 남는 선택지는 "이 프로젝트에 적용 중인 규칙셋을 배지에 'DDD' 대신 'HEXAGONAL'이라는 별도 이름으로 표시할지"뿐인데, 규칙 내용(DDD_RULE_TYPES 5종)이 100% 동일해 사용자 입장에서 **실질적 정보 이득이 없다** — 오히려 "규칙은 같은데 배지 이름만 다른" 상태가 "왜 두 프로젝트가 다른 테마로 표시되지?"라는 새로운 혼란을 만들 위험이 있다.

**판단 — 착수 보류, 자동 진행 안 함.** 이건 정확도·안전성 문제가 아니라 순수 제품/UX 판단(라벨 하나를 늘릴 가치가 있는가)이라 CLAUDE.md §1("애매하면 되물어봐라")·§7 원칙에 따라 사용자 확인 없이 혼자 결정하지 않는다. 값싼 구현(별칭 감지 로직 몇 줄 추가 + 배지 표시 분기)이라 "일단 만들고 보자"의 유혹이 있었지만, 가치가 불확실한 새 제품 개념(HEXAGONAL이라는 사용자 대면 카테고리)을 도입하는 결정 자체가 코드 난이도와 무관하게 사용자 판단 영역이라고 봤다.
...
**후속 — 독립 적대적 검증에서 발견한 저위험 갭 2건 보강 (같은 날, codeprint_159)**
PR #761/#762(pinGraph 회귀) 사고 이후, 이번 세션에 독립 적대적 검증 없이 머지된 PR들을 소급 재검증하는 과정에서 fresh-context 에이전트가 두 갭을 지적 — 둘 다 크래시·보안·오너십 문제는 아니었으나 보강할 가치가 있어 같은 세션에서 처리.
1. **O(E²) 상한 부재** — `clusterByOverlap`이 엔드포인트 쌍마다 footprint(최대 500개 라벨) 비교를 수행해 O(E²×F)인데, 엔드포인트 개수 E 자체엔 상한이 없었다. `includeMigrationGuide`가 기본 `false`(opt-in)라 일반 요청엔 영향 없지만, 대형 프로젝트에서 명시적으로 켜면 이 프로젝트가 겪은 `detect()` 콜드스타트 사고(PROGRESS.md)와 같은 패턴으로 느려질 수 있음. `MAX_VISITED`(BFS 노드 상한)와 동일한 철학으로 `MAX_ENDPOINTS_FOR_CLUSTERING=300` 추가 — 초과 시 계산 자체를 생략(크래시 대신 조용히 null, 참고용 기능이라 이 쪽이 안전). 회귀 테스트(`엔드포인트_상한초과_null`, 301개 합성 엔드포인트) 추가.
