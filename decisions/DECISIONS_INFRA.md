...
**검증.** IAM 권한 관점에서 `db-backup.yml`이 이미 쓰던 권한(PutObject·ListBucket·DeleteObject)의 완전한 부분집합만 요구(GetObject·List만)함을 확인 — 새 권한 확대 없음.

**로컬 실측 — GitHub Actions는 신규 workflow_dispatch를 main에 있어야만 인식해 머지 전 CI 트리거가 불가능**(`gh workflow run` 시도 결과 "could not find any workflows"). 대신 정확히 같은 셸/SQL 로직을 로컬 Docker로 재현: 로컬 개발 DB(48테이블·63 flyway 이력 실데이터)를 `pg_dump` → awk 필터 적용(원본 411,531줄 중 `OWNER TO` 48줄만 정확히 제거, COPY 블록 48개·종료마커 48개 전부 무손상 확인) → 별도 `postgres:18` 임시 컨테이너에 `psql` 복원(exit 0, stderr 0줄) → sanity 쿼리 실행 결과 테이블 48개·flyway 이력 63건(실패 0건) — 소스 트리의 실제 마이그레이션 파일 63개와 정확히 일치. 메커니즘 자체가 실제로 동작함을 머지 전에 확인했고, 실제 프로덕션 백업 대상 실행은 main 머지 후 workflow_dispatch로 1회 별도 실측할 것.

**프로덕션 실측(2026-07-26, main 머지 직후) — 실제 백업 대상 첫 실행에서 진짜로 실패했다.** `psql exit 3`, 파일 411,483줄 중 408,895줄(99.4% 지점)에서 `ERROR: role "codeprint_backup" does not exist`. 로컬 재현(위 항목)은 프로덕션 역할명이 로컬과 달라 이 케이스를 못 잡았던 것 — pg_dump가 파일 맨 끝에 붙이는 `ALTER DEFAULT PRIVILEGES FOR ROLE codeprint_backup ... TO codeprint_backup;`이 기존 필터(OWNER TO/GRANT/REVOKE) 어디에도 안 걸렸다.

**원인 진단 방법 — 사용자 승인 하에 1회 한정 공개 로그 상세 노출.** 실패 시 stderr를 완전히 삭제하도록 설계해뒀던 게 역으로 진단을 막아, 로컬엔 AWS CLI·자격증명이 없어 재현도 불가능했다. 사용자에게 "① 로컬에서 직접 재현 ② 1회 한해 CI 로그에 상세 에러 출력 허용 ③ 지금은 보류" 중 선택지를 제시 → ②로 결정. 임시 `debug/` 브랜치에서 stderr **앞 50줄만**(전체 대신 노출 범위 축소) `::group::`으로 감싸 노출하는 디버그 커밋을 1회 실행 — 노출된 내용은 PII 없이 role 이름(`codeprint_backup`, 내부 계정명이지 개인정보 아님) 하나뿐이었다. 원인 확인 즉시 `ALTER DEFAULT PRIVILEGES` 필터 추가 → 같은 디버그 브랜치에서 재실행해 실제 프로덕션 백업으로 완전 통과(테이블 48개·flyway 이력 63건, 실패 0건) 확인 → 진단용 verbose 로깅은 원복하고 수정된 필터만 별도 PR로 main에 반영, 디버그 브랜치·실행 로그는 정리.

**교훈.** 로컬 대체 데이터로 사전 검증해도 "실제 프로덕션 데이터에만 존재하는 패턴"(다른 role 구조, 실제 권한 부여 이력)은 못 잡을 수 있다 — 이번 워크플로의 존재 이유(백업이 진짜 복원되는지 실측)가 그 자체로 검증 도구를 검증하는 역할도 겸했다. "실패 시 상세 정보를 완전히 숨긴다"는 보안 설계가 진단 능력과 정면으로 상충할 수 있다는 것도 확인 — 완전 차단보다 "범위를 좁혀 노출"(앞 N줄만, 사용자 승인 하에 1회) 쪽이 실용적 절충점.
...
**결정.** 사용자에게 직접 질의 — 실사용자 0명·Toss 자동결제 계약 미체결 단계라 신규 비용 발생 결정은 사용자 판단이 필요하다고 보고 세 선택지(보류/저비용 우회만/상시배포 전환)를 제시. **"보류" 선택** — 상시배포 전환도, 저비용 우회(cron health check)도 지금은 착수하지 않는다. `PROGRESS.md` "🕐 대기 중" 섹션의 기존 항목("v1.0 단계 호스팅 후보")과 동일 위치에서 계속 추적, 트리거는 여전히 실사용자 유입 또는 v1.0 재논의 시점.

**되돌릴 조건.** 실사용자가 생기거나 Toss 계약이 체결되는 시점에 재논의. 재논의 시 codeprint_148의 실측(콜드스타트 16초, 캐시 미스 그래프 4개 교차 측정)을 근거로 재사용.
...
> ⚠️ **후속 조사 완료(2026-07-23, codeprint_144)** — 위 "남은 것" 두 항목 모두 확인.
>
...
**적대적 검증(fresh-context 에이전트)이 CONFIRMED 결함 2건 발견 — 같은 PR(#724)에서 수정, 머지 전.**
1. **OR/AND 오류로 무관한 취약점까지 통째로 면제됨.** 최초 구현이 "패키지 자신의 via가 acknowledged이거나, via가 참조하는 다른 패키지의 via가 acknowledged면 면제"를 `.some()`(하나라도 참이면 통과) 기준으로 판정 — 한 패키지가 **무관한 자기 자신의 취약점 + react-router를 부수적으로 참조하는 via**를 동시에 가지면, 후자만으로 전체가 면제돼버림. 에이전트가 실제로 `{via: [{url: "다른-GHSA"}, "react-router"]}`인 가짜 패키지를 만들어 스크립트에 그대로 통과(exit 0)시켜 재현. **수정**: 판정을 "via의 모든 항목이 각각 acknowledged로 설명돼야 면제"(`.every()`)로 전환.
2. **npm audit 자체가 취약점과 무관한 이유(레지스트리 장애 등)로 실패해도 false green.** `npm audit --json`이 네트워크 오류 시 `vulnerabilities` 키 없는 `{message, error}` 형태를 반환하는데, 기존 코드는 `data.vulnerabilities || {}`로 빈 객체 취급 → `Object.entries({}).length===0` → "취약점 없음"으로 오판. 에이전트가 `--registry=http://127.0.0.1:1`로 실제 재현. **수정**: `vulnerabilities` 필드 자체가 없으면 감사 실패로 간주해 즉시 `exit 1`.
3. (수정 과정에서 자체 발견) `.every()`로 재귀 판정하며 순환 참조 방어용 `seen` Set을 최상위 항목 밖에 선언했다가, 서로 다른 최상위 항목이 같은 문자열 참조를 공유하면 두 번째 항목이 "이미 방문함"으로 오판되는 걸 발견해 최상위 항목마다 새 `Set`을 만들도록 수정.
4. 4개 시나리오(실제 프로젝트 audit·에이전트의 재현 케이스·인프라 장애·원인 불명 `via: []`)를 로컬 Node로 전부 재검증 — 의도한 대로 pass/fail 갈림 확인 후 재push.

**교훈.** "화이트리스트 로직"류 보안 게이트는 "정상 케이스가 통과하는지"만 확인하면 안 되고 "무관한 걸 실수로 같이 통과시키지 않는지"를 반드시 별도로 반증해야 한다 — 이번에도 최초 구현 시 로컬 테스트는 정상 케이스만 확인했었고, 적대적 검증이 "면제 로직을 역이용하면 어떻게 되는지"를 실제로 만들어봐서야 잡혔다.
...
## CLAUDE.md에 "규칙 staleness 점검" 조항 신설 (2026-07-30, codeprint_155)

**배경.** 사용자 요청으로 유튜브 영상 2건(Boris Cherny의 "CLAUDE.md를 6개월마다 지워라", Dex Horthy의 "코드 리뷰 없이 완전 자동화한 실험 실패담")을 검토, 이 프로젝트에 적용할 게 있는지 분석. 영상2(코드 리뷰 필수)는 이미 5대 규칙·`codeprint/structure` 게이트·Plan+Checklist·PR=버티컬 슬라이스 단위로 실질적으로 구현돼 있어 추가 조치 불필요로 결론. 영상1("CLAUDE.md 전부 삭제")은 문자 그대로 적용 시 이 프로젝트가 겪은 사고 이력(G-5 웹훅 콜드스타트, BE-18 순환 빈 참조 등)을 규칙으로 증류해둔 걸 날리는 셈이라 기각했으나, "과잉 지시 vs 도메인 제약을 구분하자"는 관점 자체는 유효하다고 판단해 부분 채택.

**결정.** CLAUDE.md §7(Plan + Checklist + Context Notes + DECISIONS.md)의 기존 supersede 프로세스 바로 뒤에 "규칙 staleness 점검" 조항을 신설. 개별 규칙을 ①도메인/제품 제약(유지) ②과거 특정 시점 모델 한계를 메우던 임시방편(재검토 후보)으로 구분하는 판단 기준만 명문화하고, 실제 감사 실행은 하지 않음 — 사용자가 "복구 가능한 수준으로 점진적으로"를 명시적으로 요청해, 큰 폭의 규칙 정리를 한 번에 하는 대신 메커니즘만 작게 추가하는 쪽을 택했다.
