...
## 랜딩 템플릿 재작업 — 청사진 팔레트·스플릿 히어로·라이브 기능 비주얼·헤더 아이콘 (2026-07-28)

**문제.** 이전 커밋(페이지 통폐합)에서 만든 히어로 그래프 미리보기·버튼 마이크로인터랙션을 사용자가 "퀄리티가 떨어진다"며 재작업 대상으로 지정. 이어서 Figma A/B 시안(청사진 컬러+Stripe×Linear 중간 톤)을 실제 코드에 반영해달라는 요청 — 헤더는 A(플랫 바)가 낫고 전체 톤은 B(스플릿 히어로+글로우)가 낫다는 구체적 피드백, 기능 카드 3+2 그리드는 별로라는 피드백, 기능 설명을 "가짜 이미지 아닌 실제 페이지에서 구현될 이미지"로 채워달라는 요청.

**결정 ① — 랜딩 전용 청사진 팔레트를 Tailwind v4 `@theme`로 추가.** 전역 `gray-950` 등 기존 톤은 그대로 두고(다른 페이지 전부 리스킨은 스코프 밖), `index.css`에 `--color-blueprint-950~700` 토큰만 추가해 `bg-blueprint-950` 같은 유틸리티 클래스로 랜딩에서만 사용. 전역 리스킨과 랜딩 전용 리스킨 중 후자를 택한 이유: 이번 요청은 Figma 시안(랜딩 목업)에 한정된 것이었고, 전역으로 번지면 다른 30여 개 페이지의 회귀 확인이 필요해 스코프가 감당 안 됨.

**결정 ② — 히어로를 중앙 스택에서 좌(텍스트)/우(그래프) 스플릿으로.** 그래프 미리보기 자체도 노드 3개→4개, 배경 글로우 추가로 재작업(품질 재작업 요청 반영).

**결정 ③ — 기능 카드 그리드 폐기, 전체너비 행 교대 배치 + 라이브 미니 비주얼.** "가짜 이미지 아닌 실제 페이지 스타일"이라는 요구를 반영해, 5개 기능 각각에 대응하는 실제 화면 스타일을 그대로 축소 재현한 라이브 컴포넌트(`FeatureVisuals.tsx`) 신설 — 그래프 미리보기(구조 시각화), 이동하는 하이라이트 점(흐름 추적, CSS `offset-path` 애니메이션), 실제 `WarningPanel`과 동일한 배지 스타일(경고 감지), 부유하는 협업 커서(실시간 협업), PNG+MD 내보내기 미리보기(내보내기) 5종. 정적 스크린샷 대신 라이브 컴포넌트를 택한 이유: 이 환경엔 스크린샷 캡처 기능 자체가 없고(자동화 브라우저 탭이 프레임을 컴포지팅 못 함), 사용자도 "실제 페이지에서 구현될" 쪽을 명시적으로 선호.

**결정 ④ — 헤더 언어/테마 토글 이모지(🌐/🌙/☀️) → 선 아이콘.** "기성 이모지 쓰니까 낡아 보인다"는 피드백 반영, `AppHeader.tsx`에 `GlobeIcon`/`MoonIcon`/`SunIcon` 인라인 SVG 추가. `AppHeader`는 전역 공유 컴포넌트라 이 변경만은 랜딩 한정이 아니라 전체 페이지에 적용됨(의도된 예외 — 아이콘 노후화는 페이지 무관 문제).

**실측 중 발견한 실제 버그.** 히어로를 스플릿 레이아웃으로 바꾸며 `grid-cols-[1fr_auto]`를 썼더니 그래프 미리보기 칼럼이 CSS grid의 `auto` 트랙 크기 계산 문제로 폭 2px로 붕괴하는 버그가 있었음 — 브라우저에서 실측(`getBoundingClientRect`)하다 발견, `grid-cols-2`로 교체해 해결. `w-full` 자식이 `auto` 트랙 안에서 기준 폭을 못 잡는 전형적 패턴이라 향후 유사 레이아웃에서 참고할 것.

**검증.** `npx tsc -b`·전체 ESLint(0 errors, 기존 경고만 유지)·`npm test`(Vitest 4/4) 통과. 브라우저로 한국어·영어 렌더링, 헤더 아이콘(이모지 텍스트 미포함 확인), 히어로 그래프 4노드 겹침 없음, 기능 행 5개 좌우 교대 순서(true/false/true/false/true) 전부 실측 확인. 모바일 뷰포트(375px) 가로 스크롤 없음 확인.
...
**왜 이 셋이 다 살아남았나.** 지금까지의 카피 정정은 전부 "화면"을 단위로 돌았다 — UX 리뷰 후속(#3 랜딩 카드), AI 제거(랜딩 섹션·SettingsPage), i18n 도입(페이지 `.tsx` 30여 개 + `locales/*.json`). `index.html`은 페이지가 아니라 셸이라서 이 스윕들의 대상 목록에 한 번도 들어가지 않았다. 링크 공유 카드가 이 제품의 핵심 유입 경로인데(A2 OG 이미지를 "런칭 전 필수"로 분류한 것과 같은 이유) 정작 그 카드의 본문이 세 번의 정정을 모두 비켜간 셈이다.

**결과.** 두 메타 태그를 현재 사실 + §14.6 메시징 각도("구조 위반은 PR이 머지되기 전에 막는다")로 교체. `twitter:description`은 애초에 틀린 주장이 없어(기능 열거 없이 시각화만 서술) §3 원칙대로 손대지 않았다. 라이브 브라우저에서 실제 DOM 메타값·패치노트 렌더를 확인(v0.150.1). **재발 방지 관점**: 사용자에게 보이는 문구의 소유처가 `pages/*.tsx` + `i18n/locales/*.json` + `index.html` 셋으로 나뉘어 있다는 사실을 여기 기록해 둔다 — 다음 카피 스윕은 세 번째를 목록에 포함할 것.
