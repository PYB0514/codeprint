...
> ⚠️ **대체됨 (2026-08-02)** — "BYOK 기반 역할 명세서/기능명세(레이어A/B) — §7 supersede"로 부분 대체(아래 참조). 당시 제거 이유(화면에 이미 있는 텍스트의 재탕이라 정보이득 0)는 이번 기능엔 적용되지 않음(무주석 노드·컨텍스트 합성은 화면에 없던 새 정보) — "당분간 AI 연동 기능 없음" 방향만 번복, 개별 판단(정보이득 0)은 여전히 유효한 원칙. 아래 원문은 이력 보존용.
...
## 계정당 비공개 프로젝트 상한 6개 도입 — "무제한" 정책 뒤집기 (2026-07-28, codeprint_153)

**배경.** `contexts/Context152.md`가 뒤집은 판단(그 이전 codeprint_151에서는 "무제한 유지"로 일단락됐었음, `decisions/DECISIONS_INFRA.md` 참조)을 실제 구현으로 옮김. 저장용량만 보면 1TB 기준 2만~3.6만 프로젝트를 수용할 여유가 있어 보이지만, 사용자 5천 명만 돼도 평균 7.2개/인 수준이라 "무제한"은 무료 티어의 저장 비용을 매출 없이 무한정 키우는 구조 — 상한을 "성장 레버 마찰"이 아니라 "정상적인 프리미엄 티어링"(무료=적당량, 유료=더 많이)으로 재프레이밍하기로 방향이 정해졌었으나 구체적 숫자·구현은 codeprint_152에서 미착수였다.

**구현.**
1. **숫자 결정(사용자 확인)** — 6개로 확정(Context152가 논의한 후보 그대로). 사용자에게 "6개 vs 10개 vs 직접입력" + "백엔드만 vs 프론트 에러 메시지까지" 두 질문으로 확인 후 진행 — 프로덕션에 영향을 주는 정책 수치·UX 범위라 CLAUDE.md "애매하면 되물어라" 원칙 적용.
2. **백엔드** — `ProjectCommandService.createProject`가 `Project.create` 호출 전(신규 프로젝트는 항상 비공개로 생성되므로 이 지점이 곧 "계정당 비공개 프로젝트 상한" 체크) `projectRepository.countPrivateByUserId(userId) >= 6`이면 `IllegalStateException`(→409, 이 클래스의 기존 소유권 검증과 동일 예외 타입 재사용) 던짐. `ProjectRepository.countPrivateByUserId`는 이미 존재하던 메서드(`ProjectRepositoryImpl`이 `countByUserIdAndIsPublicFalse` JPA derived query로 실제 구현까지 완료돼 있었음, 이번에 처음 호출부만 연결) — 신규 쿼리 작성 불필요.
3. **프론트** — `CreateProjectModal.tsx`가 이미 `err.response?.data?.message ?? fallback` 패턴으로 백엔드 예외 메시지를 그대로 표시하고 있어 **프론트 코드 변경 없이** 한국어 안내 문구("비공개 프로젝트는 최대 6개까지 만들 수 있습니다. 기존 프로젝트를 공개로 전환하거나 삭제한 뒤 다시 시도해주세요.")가 자동으로 노출됨. 업그레이드 유도 CTA는 넣지 않음 — `UserPlan`이 아직 FREE/DESKTOP뿐이라(P2-0 대기 중) 유료 전환 경로 자체가 없어, 지금 만들면 갈 곳 없는 버튼이 된다.
4. **랜딩·약관 카피 동기화** — `ko/en landing.json`의 요금제 "비공개 프로젝트 무제한"을 "6개(공개는 무제한)"로, `ko/en legal.json`(이용약관 제4조)의 동일 서술도 함께 수정 — 실제 정책과 공개 문서가 어긋나지 않도록 같은 커밋에서 처리.
- **탈락한 대안**: `RepoCloner`처럼 크기 기반 상한 — 프로젝트 "개수" 자체가 정책 대상이라 무관, 애초에 고려 안 함. 상한 검사 위치를 컨트롤러로 옮기는 안 — `createProject` 안에서 URL 검증과 나란히 두는 게 "생성 가능 여부"라는 하나의 판단을 한곳에 모으는 기존 패턴(이 메서드가 이미 URL 검증을 여기서 함)과 일치해 그대로 둠.
...
## DDoS 갭① 재수정 — TOCTOU 레이스 + PR 리뷰 경로 무방비를 적대적 검증으로 발견·수정 (2026-07-30, codeprint_155)

> ⚠️ **대체: "DDoS 감사 갭① 근본 해결"(2026-07-29, codeprint_155, 같은 세션)** — 사용자가 "코드리뷰를 항상 적대적으로 하라"고 요청해 머지 완료된 #716을 fresh-context 에이전트로 소급 검증한 결과, "근본 해결"이라는 표현이 과장이었음이 CONFIRMED로 드러나 대체. 원문(통계 읽기 방식 `isAtCapacity()`)은 위 항목에 보존.

**발견된 결함(적대적 검증 CONFIRMED, 둘 다 HIGH).**
1. **PrReviewRunner(GitHub 웹훅·리컨실리에이션 cron)가 가드를 전혀 안 거침** — 같은 `taskExecutor`를 쓰는데도 `AnalysisConcurrencyGuard`는 `AnalysisApplicationService.startAnalysis`에서만 호출됐다. 원 결정 기록에 "PR 리뷰 몫으로 18 여유를 남겼다"고 적어놓고도, 그 몫을 실제로 강제하는 코드가 없었다.
2. **TOCTOU 레이스** — `isAtCapacity()`(읽기 전용 통계 확인)와 실제 `taskExecutor` 제출 사이에 GitHub API 왕복이 최대 2번 끼어 있어, 레이스 윈도우가 마이크로초가 아니라 네트워크 RTT(수백ms~수초) 단위였다. 분산 IP가 정확히 그 창 안에서 동시에 요청하면 전부 체크를 통과할 수 있었다 — 이 커밋이 막겠다던 시나리오 그 자체.

**수정.** `AnalysisConcurrencyGuard`를 통계 읽기(`isAtCapacity()`) 방식에서 `Semaphore(40)` 기반 원자적 예약(`tryAcquire()`/`release()`) 방식으로 전면 재설계.
- `AnalysisApplicationService.startAnalysis`: 레포 크기 조회 등 외부 API 호출보다 먼저 슬롯을 원자적으로 예약, 실제로 `analysisRunner.run()`에 제출하지 않고 리턴하는 모든 경로(동일 커밋 스킵·크기 초과)에서 즉시 반납. 제출에 성공하면 슬롯 소유권이 `AnalysisRunner`로 이전.
- `AnalysisRunner.run()`: 비동기 작업이 실제로 끝나는 시점(성공/실패 무관, `finally`)에 반납 — 점유 구간이 "제출"이 아니라 "실제 작업 완료"까지여야 정확하기 때문.
- `GitHubWebhookService.handle()`·`PrGateReconciliationService.reconcileProject()`: `PrReviewRunner.reviewAsync()` 호출 전에 동일한 세마포어를 예약, 실패하면 리뷰만 스킵(webhook은 정상 ACCEPTED 유지, 이미 있는 `PrGateReconciliationService`가 유실분을 다음 회차에 재시도 — 새 재시도 로직을 만들지 않고 기존 안전망을 재사용).
- `PrReviewRunner.reviewAsync()`: 작업 완료 시점(`finally`)에 반납.

**탈락한 대안.** ①통계 읽기 방식을 유지하고 임계값만 더 낮추기 — TOCTOU 자체는 해소가 안 돼 기각. ②PR 리뷰 전용 별도 세마포어 신설 — 두 경로가 물리적으로 같은 `taskExecutor` 자원을 공유하는데 감시 자원만 분리하면 여전히 서로를 못 막아 기각, 하나의 공유 세마포어가 정확한 모델.
...
## BYOK 기반 역할 명세서/기능명세(레이어A/B) — §7 supersede, PR1 착수 (2026-08-02, codeprint_158)

> **대체**: "AI 키 기반 기능 전면 제거"(2026-07-12, codeprint_117, 위 항목). 번복 이유는 원 결정의 개별 판단 오류가 아니라 새 기능이 원 결정의 전제(정보이득 0)에 안 걸리는 다른 종류라서 — 상세는 아래.

**문제.** §16 토큰절감 채널(`RepoMapService`)은 그래프에서 이미 추출된 실제 코드 주석에 의존하는데, 레거시/스파게티 코드(주석 없음)에서는 이 채널이 무력화된다. 또한 함수 단위 주석이 전부 있어도 "이 파일 여러 개가 합쳐서 무슨 기능을 구현하는가"라는 상위 추상화는 원천적으로 존재하지 않는다.

**검토 과정(사용자·Claude 3라운드 대화 + 독립 에이전트 적대적 검증 2회).**
- 1차 설계(함수 단위 무주석 노드에만 LLM 요약)는 §14.2("정보 vs 자동화")·§16.5(신념6, 구조적 사실 vs 추론) 정합성은 확인됐으나, 사용자가 "괜찮은 코드 구조 프로젝트도 이득 봐야 한다"는 요구를 추가하며 레이어B(컨텍스트 단위 기능명세, `BoundedContextResolver` 재사용)를 도입.
- 독립 에이전트 적대적 검증 1회차: Desktop(`watchLocal`)이 `sourceSets.main.runtimeClasspath`로 백엔드 전체와 같은 클래스패스에서 돈다는 사실(코드 확인)을 근거로 "웹 전용 스코프 선언"만으로는 R3(코드 외부 전송 없음) 보장이 안 됨을 지적 — DB 영구 캐시 폐기(요청 범위 생성으로 전환) + `RepoMapService.label()`의 MD 텍스트 레벨 마커 필요성으로 반영.
- 독립 에이전트 적대적 검증 2회차: `BoundedContextResolver`가 DDD 레이어 디렉터리명 휴리스틱뿐이라 비-DDD/피처슬라이스 프로젝트에서 레이어B가 실제로 발동 안 함(`RepoMapService`의 기존 null 폴백 코드로 확인)을 지적 — `GraphWarningService`의 기존 `featureOf`/`isFeatureSliceProject`(현재 `CROSS_FEATURE_IMPORT` 전용, private) 재사용 확장으로 보완, 1차 스코프는 "DDD+피처슬라이스 한정"으로 명시(레이어드/플랫 구조는 후속).
- Claude 자체 재검토로 LLM 입력 범위 정정: 처음엔 "시그니처+호출관계만"(hallucination 억제)이라 했으나, 스파게티 코드 이해라는 원목적상 함수 본문 미포함이면 정보이득이 실질적으로 낮아진다고 판단해 함수 본문 포함으로 정정(레이어A 한정, 레이어B는 기존 주석+구조요약 위주로 페이로드 작게 유지).

**결정된 설계(PR1은 이 중 BYOK 인프라만).**
- 비용: 항상 BYOK(`UserAiKey`, 서버 비용 0). 무료/유료 배치는 기능 패키징 문제로 별개(§14.2상 "정보"라 무료축 배치도 원칙상 가능, 최종 배치는 사업 판단으로 보류).
- 레이어A(함수): 무주석+HIGH_FAN_OUT/복잡도 허브 한정, 함수 본문+1홉 이웃 포함, 정보이득 근거는 "화면에 없던 요약을 새로 만든다".
- 레이어B(컨텍스트): `BoundedContextResolver`+`featureOf` 확장 스코프, 코드 품질 무관 항상 트리거(비-DDD/비-피처슬라이스는 제외), 정보이득 근거는 "개별 주석의 합이 자동으로 상위 기능 서술이 되지 않는다"(N:1 종합).
- 공통: 요청 범위 생성(DB 영구 캐시 없음, 형평성·타인 결과 영구노출 리스크 회피) — 단 프로세스 내 비영구 메모이제이션은 허용. MD 마커로 AI 생성분을 텍스트 레벨에서 실제 주석과 구분. `LocalWatcher.java`(watchLocal)는 이 서비스를 참조하지 않음(단기 `SECURITY_POLICY.md` 명문 규칙, 장기는 실제 Desktop 배포판 착수 시 Gradle 멀티모듈 분리로 컴파일 타임 강제 — 지금은 이식성만 확보).

**PR 분할**: PR1(BYOK 인프라, 이 커밋) → PR2(BoundedContextResolver `featureOf` 확장, 순수 리팩토링) → PR3(레이어A) → PR4(레이어B+A/B 교차배지).

**PR1 결과.** `UserAiKey` 도메인 엔티티·리포지토리(신규 작성 — 기존 동명 클래스는 2026-07-12 완전 삭제돼 있었음, 테이블(`user_ai_keys`, V16)만 잔존해 "재도입"이 아니라 신규 구현)+`AesEncryptionConverter` 재사용+`UserAiKeyController`(`/api/users/me/ai-key`, `SettingsPage.tsx`에 등록/삭제 UI)+i18n(ko/en). 단위테스트 5건 green, 전체 백엔드 스위트 green(Docker DB), `analyzeLocal` 베이스라인(HIGH_FAN_OUT 5) 불변, 로컬 백엔드 재기동 `/actuator/health` UP, 신규 엔드포인트 미인증 401 확인.
