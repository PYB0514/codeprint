...
## 개인정보 암호화 법적 기준 (개인정보보호법)
> 근거: 개인정보보호위원회 「개인정보의 안전성 확보조치 기준 안내서」(2025-11). 위반 시 과태료·과징금·손해배상(고의·중과실 시 최대 5배)·유출통지 의무 — 저장 시뿐 아니라 전송 시(HTTPS)도 해당.

법적으로 암호화가 의무인 항목은 **고유식별정보 4종**(주민등록번호·여권번호·운전면허번호·외국인등록번호) + **신용카드번호·계좌번호·생체인식정보 3종**, 총 7종. 이름·이메일·전화번호는 의무 대상은 아니나 유출 위험 고려해 필요 시 암호화 권장.

**현재 상태(2026-08-09 확인)** — 7종 전부 미수집:
- `User` 엔티티(`domain/user/User.java`)는 GitHub OAuth 전용 로그인이라 로컬 비밀번호·고유식별정보 자체가 없음
- 결제(Toss)는 카드·계좌번호를 PG에 위임 — `TossPaymentOrder`엔 `paymentKey`·`orderId`만 저장(카드/계좌 원본 미보유), 영상이 권장하는 "직접 저장하지 않고 위임" 패턴과 일치
- `githubAccessToken`은 저장 시 `AesEncryptionConverter`로 이미 AES 암호화 중(시크릿 관리 원칙과 별개로, DB에 남는 개인 토큰 자체의 암호화)

**신규 필드 추가 시 판단 기준**: 위 7종 중 하나에 해당하면 ① `AesEncryptionConverter` 패턴 재사용해 암호화하거나 ② 애초에 저장하지 않고 PG·본인확인기관 등에 위임 — 평문 저장 금지.
...
| `PUT /api/users/me/ai-key`·`DELETE /api/users/me/ai-key/*`·`PUT /api/users/me/ai-key/priority` | IP당 10회/분(세 엔드포인트가 `ai-key-write` 버킷 공유) | BYOK 키 등록/삭제/우선순위 재배열 — 인증이 1차 방어선, 세션 탈취·남용 시 무제한 반복 쓰기 방지가 목적(BYOK 시리즈 PR1 신설 당시 누락됐던 갭, 2026-08-02 발견) |
...
[ ] 신규 사용자 데이터 필드가 개인정보보호법 암호화 의무 7종(고유식별정보 4종+카드/계좌/생체 3종)에 해당하면 암호화하거나 애초에 저장하지 않고 위임했는가(위 "개인정보 암호화 법적 기준" 섹션 참조)
```
