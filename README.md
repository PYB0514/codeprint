...
[![Live](https://img.shields.io/badge/Live-codeprint.dev-2563eb)](https://codeprint.dev)
...
Codeprint는 GitHub 레포를 정적 분석해 **파일·함수·DB·API를 노드로, 호출/임포트/DB접근/API호출을 엣지로** 그린 인터랙티브 그래프를 만든다. 여기에 더해, 정적 분석으로 잡을 수 있는 **런타임·아키텍처 문제 패턴**(순환 의존, 끊긴 인터페이스 체인, DDD 경계 위반, DB 레이어 우회 등)을 자동 감지하고, **GitHub PR에 경고를 코멘트로 달아 머지 전에 알려준다.**

- **웹 서비스** — GitHub PR 연동: PR을 열면 변경된 브랜치를 분석해 구조 경고를 자동 코멘트.
- **(예정) 데스크탑 앱** — 로컬 폴더 분석: 코드를 외부로 전송하지 않고 push 전에 로컬에서 경고 감지.

🔗 **Live:** [codeprint.dev](https://codeprint.dev)
...
### 분석 엔진
- **다국어 정적 분석** — 정규식 기반 분석기로 11개 언어(Java·Kotlin·TS/JS·Python·Go·C#·Ruby·PHP·Swift 등) 지원.
...
### 구조 경고 감지 (차별화 포인트)
정적 분석으로 런타임·아키텍처 문제를 선제 감지하고, severity(HIGH/MEDIUM/LOW)로 분류한다.
- 순환 의존(`CYCLIC_IMPORT`) · 끊긴 인터페이스 체인(`BROKEN_INTERFACE_CHAIN`) · `@Async` 자기 호출 · DB 레이어 우회
- DDD 경계 위반(`CROSS_DOMAIN_CALL`·`CROSS_CONTEXT_IMPORT`·`DOMAIN_IMPORTS_INFRA`) · `@Convert` 마이그레이션 누락 · Dead Code · High Fan-out
- **오탐 캘리브레이션** — 실제 오픈소스 레포(Spring PetClinic, requests 등)로 오탐률을 벤치마크해 신뢰도 게이트를 조정.
...
- [x] 다국어 분석 엔진 · 구조 경고 감지 + severity · 오탐 캘리브레이션
- [x] React Flow 시각화 · 흐름 재생 · 도메인/계층 이중 뷰 · 버전 보관 정책
- [x] 실시간 협업 · 커뮤니티 · AI 연동 · MCP 컨텍스트 엔드포인트
- [x] GitHub PR 연동 (수동 트리거) · 결제 · 관리자 대시보드
- [ ] **GitHub PR webhook 자동화** (PR 열림/푸시 시 자동 리뷰) — *진행 중*
- [ ] "슈퍼 바이브 코딩" — 그래프 위 설계 스케치 레이어 · 인앱 AI 어시스트
- [ ] 데스크탑 앱(Electron) — 로컬 폴더 분석, 코드 외부 전송 없음
...
**Codeprint** · 1인 풀스택 개발 · [codeprint.dev](https://codeprint.dev)
