...
| 흐름 자동 시각화 프로토타입 | 🚧 | feat/flow-playback — 재생 컨트롤 + 노드/엣지 하이라이트 동작 확인 |
...
```
# 현재: feat/flow-playback 브랜치 (PR 미오픈, 프로토타입 완성)
# 1. 흐름 재생 개선 3가지 작업 후 PR → main 머지
#    - 재생 시 관련 엣지 전부 표시 (on/off 상태 무관)
#    - 재생 시 fitView로 경로 노드 화면 안에 맞추기
#    - 추후: DB_TABLE 노드 추출 (JPA @Entity, Prisma 파싱)
# 2. DB 구조 시각화 — 코드에서 직접 추출 (보안 이슈 없는 방향)
```
