...
> "도그푸딩"이라는 말이 이 프로젝트에서 두 가지 서로 다른 결로 쓰인다 — 헤깔리지 않도록 한 표로 정리.

| 종류 | 무엇을 스스로 쓰는가 | 증거·위치 |
|---|---|---|
| **① 게이트 자기적용** | `codeprint/structure`가 Codeprint 자신의 PR도 검사(신념 4 "우리에게 먼저 적용한다") | 아래 "★ 대표 사례" 이하 전체 — 자기 코드에서 실제 결함을 적발한 기록 |
| **② 로컬 도구(에이전트용)** | AI 에이전트가 코드 탐색·자가검사 시 자체 도구를 LLM의 재해석보다 먼저 씀(신념 6) | `exploreLocal`(repoMap/find/neighbors) · `analyzeLocal`(push 전 워닝 자가검사) — CLAUDE.md §0에서 강제(훅으로 Grep/Glob 우회 차단) |
| **③ 자기분석 웹 프로젝트** | "codeprint" 프로젝트로 자기 코드베이스를 실제 그래프로 시각화·경고 확인 | 프로덕션 웹에 일반 프로젝트로 등록돼 있음 |
| **④ 워치모드** | `watchLocal`로 저장 시 자동 재검사(개발 중 실시간 피드백) | PROGRESS.md "데스크탑 워치모드" |
| **⑤ 엣지 감사 도구** | `edgeAudit`으로 자기 그래프의 엣지 정확도를 직접 표본 검증 | `backend/src/test/resources/edge-audit/self/` |

①은 "제품이 우리 결함을 잡는다"(품질 증거), ②는 "에이전트가 우리 도구를 자기 추론보다 우선한다"(제품 자체를 컨텍스트 소스로 실사용, §16). 이 파일의 이하 기록은 ①만 다룬다 — ②~⑤는 위 표가 유일한 요약이다.
